...
class Softmax_Network(nn.Module):
    """
    The Deep Q-Network (Softmax) model for reinforcement learning.
    """

    def __init__(self, num_actions, input_dim):
        """
        Initialize the Softmax network.
        Parameters:
...
class Softmax_Agent:
    """
    Softmax Agent Class. This class defines some key elements of the Softmax algorithm,
    ...
    def __init__(self, env,
                 clip_grad_norm, learning_rate, discount, memory_capacity,tau=1.0,):
        ...
        self.main_network = Softmax_Network(num_actions=self.action_space.n, input_dim=self.observation_space.shape[0]).to(device)
        self.target_network = Softmax_Network(num_actions=self.action_space.n, input_dim=self.observation_space.shape[0]).to(
            device).eval()
    ...
    def select_action(self, state):
        """
        select actions based on softmax policy
        """
        with torch.no_grad():
             Q_values = self.main_network(state)
             probabilities = F.softmax(Q_values, dim=-1)  # Apply softmax directly on Q-values
...
class Model_TrainTest:
    def __init__(self, hyperparams):
        ...
        self.agent = Softmax_Agent(env=self.env,
    ...
    def plot_training(self, episode):
        ...
        if episode == self.max_episodes:
            plt.savefig('d:\\term8\Deep Reinforcement Learning\HWs\HW1\Softmax/reward_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('d:\\term8\Deep Reinforcement Learning\HWs\HW1\Softmax\Loss_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "RL_load_path": 'd:\\term8\Deep Reinforcement Learning\HWs\HW1\Softmax\\' + '_' + '500' + '.pth',
        "save_path":'d:\\term8\Deep Reinforcement Learning\HWs\HW1\Softmax/',
        "save_interval": 500,
        ...
        "batch_size": 64,
        "update_frequency": 10,
        "max_episodes": 500 if train_mode else 10,
        "max_steps": 500,
        "render": render,
