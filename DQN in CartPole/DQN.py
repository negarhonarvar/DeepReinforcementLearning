...
class DQN_Agent:
    ...
    def __init__(self, env, epsilon_max, epsilon_min, epsilon_decay,
        ...
        self.target_network = DQN_Network(num_actions=self.action_space.n, input_dim=self.observation_space.shape[0]).to(
            device).eval()
...
class Model_TrainTest:
    ...
    def plot_training(self, episode):
        ...
        if episode == self.max_episodes:
            plt.savefig('d:\\term8\Deep Reinforcement Learning\HWs\HW1\DQN/Epsilon_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('d:\\term8\Deep Reinforcement Learning\HWs\HW1\DQN/reward_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
        ...
        # Only save as file if last episode
        if episode == self.max_episodes:
            plt.savefig('d:\\term8\Deep Reinforcement Learning\HWs\HW1\DQN\Loss_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "RL_load_path": 'd:\\term8\Deep Reinforcement Learning\HWs\HW1\DQN\DQN' + '_' + '2000' + '.pth',
        "save_path":'d:\\term8\Deep Reinforcement Learning\HWs\HW1\DQN',
        "save_interval": 500,
