    ...
    def Learn(self, memory):
        ...
        for _ in range(self.K_epochs):
            for i in range(0, len(old_states), self.batch_size):
                ...
                clipped_ratios = torch.clamp(ratios, 1.0 - self.eps_clip, 1.0 + self.eps_clip)
                surr1 = ratios * advantages
                surr2 = clipped_ratios * advantages
                actor_loss = -torch.min(surr1, surr2).mean()
                critic_loss = self.MseLoss(state_values.squeeze(), sampled_rewards)
                entropy = dist.entropy().mean()
                loss = actor_loss + self.mse_coef * critic_loss - self.entropy_coef * entropy
...
def train(hyperparams, render=False):
    ...
    for i_episode in range(0, 501):
        ...
        if i_episode % 500 == 0:
            ...
            plt.ylabel("Rewards")
            plt.legend()
            plt.savefig('./eps=0.4reward_plot.png', format='png', dpi=600, bbox_inches='tight')
            plt.tight_layout()
            plt.grid(True)
            plt.show()
            plt.clf()
            ...
            plt.ylabel("Loss")
            plt.legend()
            plt.savefig('./eps=0.4Loss_plot.png', format='png', dpi=600, bbox_inches='tight')
            plt.tight_layout()
            plt.grid(True)
            plt.show()
            plt.clf()
