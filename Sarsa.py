...
class QTable:
    def __init__(self, observation_space, action_space, bin_size=10):
        # Initialize bins
        self.bins = [
            np.linspace(-4.8, 4.8, bin_size),  # For Cart Position
            np.linspace(-4, 4, bin_size),      # For Cart Velocity
            np.linspace(-0.418, 0.418, bin_size), # For Pole Angle
            np.linspace(-4, 4, bin_size)       # For Pole Angular Velocity
        ]
...
class SARSA:
    def __init__(self, env, q_table, policy, Learning_rate=2.3e-3, Discount_factor=0.999, max_episode=5000, max_step=500, render=False):
        self.env = env
        self.q_table = q_table
        self.policy = policy
    ...
    def train(self):
        ...
        for episode in range(self.max_episode+1):
            state, _ = self.env.reset()  # Unpack the tuple to ignore the second part
...
policy = EpsilonGreedyPolicy(0.999, 0.999, 0.01, q_table.q_table)
...
# test_rewards = sarsa_agent.test(model_path, num_episodes=20)
