...
class DQN_Agent:
    ...
    def __init__(self, state_size=8, action_size=4, hidden_size=64,
                 learning_rate=1e-3, discount_factor=0.99, buffer_size=10000, batch_size=64):
        ...
        self.memory = Replay_Memory(buffer_size)
...
def train(agent, env, max_episodes=2000, epsilon_max=1.0, epsilon_min=0.01, epsilon_decay=0.995, target_update=10):
    ...
    for current_episode in range(1, max_episodes + 1):
        ...
        print(f"\rEpisode {current_episode}\tAverage Score: {np.mean(final_scores):.2f}", end="")
        if current_episode % target_update == 0:
            agent.hard_update()
        if current_episode % 100 == 0:
            print('\rEpisode {}\tAverage Score: {:.2f}'.format(current_episode, np.mean(final_scores)))

        if current_episode % 100 == 0 and np.mean(final_scores) >= 200:
            save_path = 'd:\\term8\Deep Reinforcement Learning\HWs\HW2\DQN'
            agent.save(save_path + '_' + f'{current_episode}' + '.pth')
            break

    plt.figure()
    plt.title("Average Q of target Network")
    plt.plot(final_scores, label='Raw Reward', color='#FF48B0', alpha=1)
    plt.xlabel("Episode")
